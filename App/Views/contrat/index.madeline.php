                    ...
                    <tr class="group hover:bg-slate-50/50 transition-all cursor-pointer" onclick="window.location='/contrats/edit/{{ $ctr->id }}'">
                        ...
                        <td class="py-8">
                            <div class="flex items-center gap-3">
                                ...
                                @ndax($ctr->is_read)
                                <div class="w-2 h-2 rounded-full bg-amber-500 animate-pulse" title="Consulté par la partie adverse"></div>
                                @jeexndax
                            </div>
                        </td>
                        <td class="py-8 text-right">
                            <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <a href="/view/contrat/{{ $ctr->token_public }}" target="_blank" class="p-3 rounded-xl bg-slate-100 text-slate-400 hover:text-amber-600 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                </a>
                                <a href="/contrats/edit/{{ $ctr->id }}" class="p-3 rounded-xl bg-slate-100 text-slate-400 hover:text-slate-900 transition-colors">
