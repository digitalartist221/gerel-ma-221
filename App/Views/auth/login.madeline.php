...
@def('pageTitle')Connexion — Gerel Ma Business@jeexdef

@def('content')
<div class="min-h-[70vh] flex items-center justify-center pt-32 pb-20">
    <div class="max-w-md w-full floating-card p-12 rounded-6xl border-gray-100">
        ...
        @ndax(isset($_GET['registered']))
            <div class="bg-emerald-50 text-emerald-600 text-[11px] font-bold uppercase tracking-widest p-4 rounded-2xl mb-8 border border-emerald-100 text-center flex items-center justify-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                Compte créé ! Vous pouvez maintenant vous connecter.
            </div>
        @jeexndax

        @ndax(isset($_GET['reset']))
            <div class="bg-emerald-50 text-emerald-600 text-[11px] font-bold uppercase tracking-widest p-4 rounded-2xl mb-8 border border-emerald-100 text-center flex items-center justify-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
        ...
        <form method="POST" action="/login" class="space-y-6">
            @csrf
            <div>
                <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2 ml-4">Adresse Email</label>
                <input type="email" name="email" required placeholder="nom@entreprise.com" class="w-full px-6 py-4 rounded-full bg-gray-50 border border-gray-100 focus:bg-white focus:ring-2 focus:ring-brand-500 focus:outline-none transition-all text-sm font-medium">
            </div>
            <div>
                <div class="flex items-center justify-between mb-2 ml-4 mr-1">
                    <label class="block text-[10px] font-black uppercase tracking-widest text-gray-400">Mot de passe</label>
                    <a href="/forgot-password" class="text-[10px] font-black uppercase tracking-widest text-brand-500 hover:underline">Oublié ?</a>
                </div>
